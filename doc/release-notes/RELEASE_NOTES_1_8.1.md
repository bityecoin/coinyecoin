# Coinyecoin Core 1.8.1

Coinyecoin Core 1.8.1 is primarily a bugfix release, bringing Coinyecoin Core in
line with Bitcoin 0.9.3. Coinyecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Coinyecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Coinyecoin Core clients
* Use HTTPS with coyechain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/coinyecoin/coinyecoin/issues/134).
