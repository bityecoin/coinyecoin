### Building coinyecoin-qt 1.14 on modern macs. ###
...
### Clone coinyecoin locally, or check it out, etc. ###

For this purpose, just indicating the 1.14-branding branch in my repo.

	$git clone -b 1.14-branding --single-branch https://github.com/michilumin/coinyecoin.git
...
Patch bdb 5.1.29 from our patchfiles
    ...
    $cd src
    $cd dbinc
    $patch -b atomic.h ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/atomic.h.patch
    $cd ..
    $cd mp
    $patch -b mp_fget.c ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/mp_fget.c.patch
    $patch -b mp_mvcc.c ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/mp_mvcc.c.patch
    $patch -b mp_region.c ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/mp_region.c.patch
    $cd ..
    $cd mutex
    $patch -b mut_method.c ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/mut_method.c.patch
    $patch -b mut_tas.c ~/coinyecoin/depends/patches/bdb-5.1.29-clang-osx/mut_tas.c.patch
...
### Go back to your Coinyecoin repo ###
