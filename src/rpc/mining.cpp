...
static bool AllowStaleMining()
{
    return GetBoolArg("-allowstalemining", false);
}
...
{
    // Coinyecoin: Never mine witness tx
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        // Coinyecoin: Don't mine Aux blocks in regtest
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated coinyecoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && !AllowStaleMining())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Bitcoin is not connected!");

    if (IsInitialBlockDownload() && !AllowStaleMining())
...
UniValue getauxblockbip22(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 &&
        !Params().MineBlocksOnDemand() && !AllowStaleMining())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Coinyecoin is not connected!");

    if (IsInitialBlockDownload() && !Params().MineBlocksOnDemand() && !AllowStaleMining())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Coinyecoin is downloading blocks...");
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            "getgenerate\n"
            "\nReturn if the server is set to generate coins with the built-in CPU miner or not.\n"
            "Set with -gen at startup or with the setgenerate call.\n"
            "\nResult\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nSet 'generate' true or false to turn the built-in CPU miner on or off, old-school style.\n"
            "Generation is limited to 'genproclimit' processors, -1 is unlimited.\n"
            "Coins are paid to -miningaddress if set, otherwise to the local wallet.\n"
            "\nArguments:\n"
            "1. generate         (boolean, required) Set to true to turn on generation, false to turn off.\n"
            "2. genproclimit     (numeric, optional) Set processor limit when generation is on. Can be -1 for unlimited.\n"
            "\nExamples:\n"
            "\nTurn generation on with one thread\n"
    ...
    GenerateCoinyecoins(fGenerate, nGenProcLimit, Params());
    return NullUniValue;
}

UniValue getstratuminfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            "getstratuminfo\n"
            "\nReturns status of the built-in stratum server (start with -stratum=1).\n"
            "\nExamples:\n"
            + HelpExampleCli("getstratuminfo", "")
            + HelpExampleRpc("getstratuminfo", ""));
    return GetStratumInfo();
}
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "mining",             "getstratuminfo",         &getstratuminfo,         true,  {} },
