    ...
    platformStyle(_platformStyle)
{
    ...
    // Coinyecoin: load fallback font in case Comic Sans is not availble on the system
    ...
    // Coinyecoin: Specify Comic Sans as new font.
    QFont newFont("Comic Sans MS", 10);

    // Coinyecoin: Set new application font
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Coinyecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and coinyecoin: URIs)"));
    ...
    miningAction->setStatusTip(tr("Mine COYE with your CPU"));
    ...
    openAction->setStatusTip(tr("Open a coinyecoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Coinyecoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Coinyecoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    // Fishsticks: on a long-idle chain, treat the node as fully synced once
    // every block announced by peers has been validated (the tip has caught
    // up to the best known header), even if that block is months old.
    bool fCaughtUpToHeaders = !header && count >= clientModel->getHeaderTipHeight();
...
{
    QString strTitle = tr("Coinyecoin"); // default title
