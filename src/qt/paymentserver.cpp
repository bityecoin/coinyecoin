...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    // Use these certificates when fetching payment requests, too.
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start coinyecoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        {
            QByteArray temp = uri.queryItemValue("r").toUtf8();
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Coinyecoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
