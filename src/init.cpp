...
static boost::signals2::connection g_notify_block_change_connection;
static boost::signals2::connection g_genesis_wait_connection;
static boost::signals2::connection g_block_notify_callback_connection;
...
void Shutdown()
{
    ...
    RenameThread("coinyecoin-shutoff");
    ...
    StopRPC();
    StopHTTPServer();
    // Fishsticks: stop solo mining services before tearing down validation
    InterruptStratumServer();
    StopStratumServer();
    GenerateCoinyecoins(false, 0, Params());
#ifdef ENABLE_WALLET
    // Coinyecoin 1.14 TODO: ShutdownRPCMining();
    if (pwalletMain)
...
void OnRPCStarted()
{
    g_notify_block_change_connection = uiInterface.NotifyBlockTip.connect(&RPCNotifyBlockChange);
}

void OnRPCStopped()
{
    g_notify_block_change_connection.disconnect();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-allowstalemining", _("Allow getblocktemplate/getauxblock to serve mining work even while the local tip is stale or initial block download is detected (default: 0)"));
    strUsage += HelpMessageOpt("-gen", _("Generate coins with the built-in CPU miner (default: 0)"));
    strUsage += HelpMessageOpt("-genproclimit=<n>", _("Number of threads for the built-in CPU miner, -1 = all cores (default: 1)"));
    strUsage += HelpMessageOpt("-miningaddress=<addr>", _("Pay built-in CPU miner rewards to <addr> instead of the local wallet"));
    strUsage += HelpMessageOpt("-stratum", _("Run the built-in stratum server so external miners (ccminer, cpuminer, cgminer, Antminer) can solo mine straight against this wallet (default: 0)"));
    strUsage += HelpMessageOpt("-stratumbind=<ip>", _("Bind the stratum server to <ip> (default: 0.0.0.0)"));
    strUsage += HelpMessageOpt("-stratumport=<n>", _("Listen for stratum connections on <port> (default: 3333)"));
    strUsage += HelpMessageOpt("-stratumaddress=<addr>", _("Pay stratum miners to <addr> when their worker name is not a valid Coinyecoin address (default: local wallet)"));
    strUsage += HelpMessageOpt("-stratumdiff=<d>", _("Starting share difficulty for stratum clients (default: 16)"));
    strUsage += HelpMessageOpt("-stratumvardiff", _("Automatically retarget stratum share difficulty per miner (default: 1)"));
    strUsage += HelpMessageOpt("-stratummaxconn=<n>", _("Maximum simultaneous stratum client connections (default: 64)"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bityecoin/coinyecoin>";
    const std::string URL_WEBSITE = "<https://coinye.info>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("coinyecoin-loadblk");
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Coinyecoin version %s\n", FormatFullVersion());
}
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (chainActive.Tip() == NULL) {
        g_genesis_wait_connection = uiInterface.NotifyBlockTip.connect(BlockNotifyGenesisWait);
    } else {
        fHaveGenesis = true;
    }

    if (IsArgSet("-blocknotify"))
        g_block_notify_callback_connection = uiInterface.NotifyBlockTip.connect(BlockNotifyCallback);
    ...
    // Wait for genesis block to be processed
    {
        ...
        g_genesis_wait_connection.disconnect();
    }
    ...
    // Coinyecoin: Do we need to do any RPC mining init here?

    SetRPCWarmupFinished();
    uiInterface.InitMessage(_("Done loading"));

    // ********************************************************* Fishsticks: built-in solo mining
    if (GetBoolArg("-stratum", DEFAULT_STRATUM_ENABLE)) {
        if (!StartStratumServer())
            return InitError(_("Failed to start stratum server (check -stratumaddress / -stratumbind / -stratumport and debug.log)"));
    }
    if (GetBoolArg("-gen", false))
        GenerateCoinyecoins(true, GetArg("-genproclimit", 1), chainparams);
