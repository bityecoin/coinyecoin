...
bool IsInitialBlockDownload()
{
    ...
    const bool allowStaleMining = GetBoolArg("-allowstalemining", false);
    ...
    if (chainActive.Tip() == NULL)
        return true;
    if (!allowStaleMining &&
        chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus(chainActive.Height()).nMinimumChainWork))
        return true;
    if (!allowStaleMining &&
