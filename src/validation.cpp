...
#if defined(NDEBUG)
# error "Coinyecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Coinyecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetCoinyecoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool IsInitialBlockDownload()
{
    ...
    // Fishsticks stale-chain fix: a node only reports "initial block download"
    // while it knows of a better chain (via peer headers) than it has fully
    // validated. Once the active tip has caught up to the best known header,
    // the node is as synced as the network allows, even if that tip is months
    // old. Wallet, RPC and mining all become usable again on a long-idle
    // chain. Local policy only; consensus rules are untouched.
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Coinyecoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("coinyecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Coinyecoin: BIP30 has been active since inception
    ...
    // Coinyecoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetCoinyecoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Coinyecoin: Disallow AuxPow blocks before it is activated.
    ...
    // Coinyecoin: Version 2 enforcement was never used
    ...
    // Coinyecoin: Introduce supermajority rules for v4 blocks
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Coinyecoin: We probably want to disable this
    int nLockTimeFlags = 0;
