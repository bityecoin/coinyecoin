...
{
    // Stratum sends the previous block hash with each 4-byte word byte-swapped,
    // word order preserved. Miners (cpuminer/ccminer/cgminer and every scrypt
    // ASIC) byte-swap each word again to rebuild the header field, recovering
    // the internal hash. Getting this wrong makes every submitted share hash a
    // different header than the pool reconstructs, so all shares are rejected
    // as "above target" even though the miner is working correctly.
    const unsigned char* p = hash.begin();   // internal (little-endian) order
    std::string str;
    str.reserve(64);
    for (int i = 0; i < 8; ++i) {
        for (int j = 3; j >= 0; --j)
            str += strprintf("%02x", p[4 * i + j]);
    }
    return str;
}
