...
// Coinyecoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateCoinyecoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetCoinyecoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetCoinyecoinDustFee(tx.vout, ::minRelayTxFee);
